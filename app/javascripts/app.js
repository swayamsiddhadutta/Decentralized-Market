  ...
  if($("#product-details").length > 0){
   ...
   $("#buy-now").submit(function(event){
      ...
      EcommerceStore.deployed().then(function(i){
        i.buy(productId,{value:sendAmount,from: web3.eth.accounts[0],gas:44400}).then(function(f){
              $("#msg").show();
...
function renderProductDetails(productId){
  EcommerceStore.deployed().then(function(f){
    f.getProduct.call(productId).then(function(p){
      ...
      $("#product-image").html("<img width='500' src = 'http://localhost:8080/ipfs/" + p[3] + "' />");
...
function saveProduct(product){
  ...
  saveImageOnIpfs(reader).then(function(id){
    ...
    web3.toWei(product["product-price"],'ether'),product["product-condition"],
...
function renderProduct(instance,index){
  instance.getProduct.call(index).then(function(f){
    ...
    node.append("<div>" + displayPrice(f[6]) + "</div>");
