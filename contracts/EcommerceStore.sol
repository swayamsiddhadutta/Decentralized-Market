...
contract EcommerceStore {
    ...
    function EcommerceStore(address _arbiter) public{
        productIndex = 0;
        arbiter = _arbiter;
    }
    function addProductToStore(string _name,string _category,string _imageLink,
         string _desclink,uint _startTime,uint _price,uint _productCondition) public {
             ...
             NewProduct(productIndex, _name, _category, _imageLink, _desclink, _startTime, _price, _productCondition);   
         }
    ...
    function getProduct(uint _productId) public view returns(uint,string,string,string,string,uint,uint,ProductCondition,address) {
        ...
        return (product.id,product.name,product.category,product.imageLink,product.descLink,product.startTime,
